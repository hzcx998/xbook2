...
iostatus_t serial_read(device_object_t *device, io_request_t *ioreq)
{
    ...
    printk(KERN_DEBUG "serial_write: %s\n", buf);
#endif
    ioreq->io_status.status = IO_SUCCESS;
...
static iostatus_t serial_enter(driver_object_t *driver)
{
    ...
    for (id = 0; id < MAX_COM_NR; id++) {
        ...
        if (status != IO_SUCCESS) {
            printk(KERN_ERR "serial_enter: create device failed!\n");
            return status;
        }
        /* buffered io mode */
...
#ifdef DEBUG_DRV
        printk(KERN_DEBUG "serial_enter: device extension: device name=%s object=%x\n",
        ...
        devext->irq = irq;
#ifdef DEBUG_DRV
        printk(KERN_DEBUG "serial_enter: com%d, base:%x irq:%d\n", id, iobase, irq);
#endif  /* DEBUG_SERIAL */
...
#ifdef DEBUG_DRV
    printk(KERN_DEBUG "serial_driver_func: driver name=%s\n",
        driver->name.text);
#endif
    return status;
}
