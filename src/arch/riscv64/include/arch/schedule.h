...
typedef struct {
    ...
    proc_t *idle;           /* 当前调度单元的idle任务 */
    proc_t *cur;            /* 当前调度单元的执行中的任务 */
...
static inline int sched_queue_has_task(sched_unit_t *su, proc_t *task)
{
    sched_queue_t *queue = &su->priority_queue[su->dynamic_priority];
    return list_find(&task->list, &queue->list);
}

static inline void sched_queue_add_tail(sched_unit_t *su, proc_t *task)
{    
...
static inline void sched_queue_add_head(sched_unit_t *su, proc_t *task)
{
    sched_queue_t *queue = su->priority_queue + task->priority;
    assert(!list_find(&task->list, &queue->list));
    list_add_tail(&task->list, &queue->list);
