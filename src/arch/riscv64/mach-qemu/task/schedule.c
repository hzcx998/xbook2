...
uint8_t sched_calc_new_priority(proc_t *task, char adjustment)
{
...
static proc_t *sched_queue_fetch_first(sched_unit_t *su)
{
    proc_t *task;
    /* 总是选择优先级比较高的队列 */
    ...
    task = list_first_owner(&queue->list, proc_t, list);
    --queue->length;
    --su->tasknr;
...
proc_t *get_next_task(sched_unit_t *su)
{
    proc_t *task = su->cur;
    switch (task->state) {
    case TASK_RUNNING:
...
static void sched_set_next_task(sched_unit_t *su, proc_t *next)
{
    fpu_save(&su->cur->fpu);
    su->cur = next;
    proc_activate_when_sched(su->cur);
