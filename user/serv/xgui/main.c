...
int main(int argc, char *argv[])
{
    ...
    server_xgui_render_viewfill(view0, XGUI_RED);
    xgui_view_set_z(view0, 0);

    xgui_view_t *view = xgui_view_create(100, 100, 400, 300);
    server_xgui_render_viewfill(view, XGUI_BLUE);
    
    printf("top z:%d\n", xgui_view_get_top()->z);
    xgui_view_move_upper_top(view);

    view = xgui_view_create(300, 300, 400, 300);
    server_xgui_render_viewfill(view, XGUI_GREEN);
    server_xgui_render_line(view, 0, 0, 100, 150, XGUI_YELLOW);
    server_xgui_render_rect(view, 50, 50, 50, 100, XGUI_BLACK);
    server_xgui_render_vline(view, 200, 50, 200, XGUI_RED);
    server_xgui_render_hline(view, 50, 200, 200, XGUI_RED);
    server_xgui_render_rectfill(view, 100, 150, 100, 50, XGUI_WHITE);

    server_xgui_render_text(view, 100, 50, "hello, world!\n", XGUI_GRAY);

    server_xgui_render_text(view, 200, 100, "hello, world!\n", XGUI_BLACK);
    ...
    server_xgui_render_bitblt(view, 0, 0, bmp, 10,10,32,32);
    printf("top z:%d\n", xgui_view_get_top()->z);
    xgui_view_move_upper_top(view);
    ...
    server_xgui_render_bitblt(view, 0, 0, &ibmp, 0,0,iw, ih);
    free(ibuf);
