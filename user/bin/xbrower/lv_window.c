...
static int do_register_drv(uint32_t width, uint32_t height)
{
    ...
    lv_color_t *buf = malloc((width * height * sizeof(lv_color_t)));                     /*Declare a buffer for 1/10 screen size*/
    if (buf == NULL) {
        lv_display_exit();
        return -1;
    }

    lv_disp_buf_init(&disp_buf, buf, NULL, width * height);    /*Initialize the display buffer*/    
