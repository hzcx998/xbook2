...
int  in_output_screen_pixel_abs(HDC hdc, int x, int y, SCREEN_COLOR  color)
{
    ...
    GUI_COLOR     gback_color = GUI_BLACK;
    GUI_COLOR     gfore_color = GUI_BLACK;
    ...
    #ifdef  _LG_WINDOW_
    /* cur_clip_rect */
    rect = ((HWND)(((HDC)hdc)->hwnd))->common.cur_clip_rect;
    #else
    rect = ((HDC)hdc)->rect;
    #endif
    ...
    ret = in_point_input_pixel_abs(hdc, x, y, &gback_color);
    if ( ret < 1 )
    {
        return  -1;
    }

    gfore_color = (lscrn.screen_to_gui_color)(color);

    tmp_color = 0;
    if ( (hdc->alpha_blend_op_mode) == ALPHA_BLEND_OP_SUBTRACT )
    {
        /* Blue */
        tmp1_color = (gback_color&0xFF);  
        tmp2_color = (gfore_color&0xFF);  
        if ( tmp1_color > tmp2_color )
            tmp_color |= tmp1_color - tmp2_color;

        /* Green */
        tmp1_color = (gback_color&0xFF00);  
        tmp2_color = (gfore_color&0xFF00);  
        if ( tmp1_color > tmp2_color )
            tmp_color |= tmp1_color - tmp2_color;

        /* RED */
        tmp1_color = (gback_color&0xFF0000);  
        tmp2_color = (gfore_color&0xFF0000);  
        if ( tmp1_color > tmp2_color )
            tmp_color |= tmp1_color - tmp2_color;
        ...
        /* Green */
        tmp1_color = ((gback_color&0xFF00)>>8);  
        tmp2_color = ((gfore_color&0xFF00)>>8);
        tmp3_color = (((tmp1_color*alpha_value) + tmp2_color*(256 - alpha_value)) >> 8);
        tmp_color |= ((tmp3_color&0xFF)<<8);
 
        /* RED */
        tmp1_color = ((gback_color&0xFF0000)>>16);
        tmp2_color = ((gfore_color&0xFF0000)>>16);
        tmp3_color = (((tmp1_color*alpha_value) + tmp2_color*(256 - alpha_value)) >> 8);
        tmp_color |= ((tmp3_color&0xFF)<<16);
    }

    color = (lscrn.gui_to_screen_color)(tmp_color);
    #endif
...
int  in_point_input_pixel_abs(HDC hdc, int x, int y, GUI_COLOR  *color)
{
    ...
    /* Bound ?*/
    /* x < 0 or x > width - 1 */
    ret = (lscrn.input_pixel)(x, y, &screen_color);
    if ( ret < 0 )
	return  -1;
...
int  point_input_pixel_abs(HDC hdc, int x, int y, GUI_COLOR  *color)
{
    ...
    gui_lock( );
    ret = in_point_input_pixel_abs(hdc, x, y, color);
    gui_unlock( );
